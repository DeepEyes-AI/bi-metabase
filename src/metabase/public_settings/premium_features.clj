...
(mu/defn ^:private fetch-token-status* :- TokenStatus
  ...
  (if (mc/validate ValidToken token)
    (do
      {:valid         true
       :status        "Activate"
       :features ["whitelabel", "audit-app", "advanced-permissions", "embedding", "cache-granular-controls", "sso-google", "sso-jwt", "disable-password-login", "dashboard-subscription-filters", "official-collections", "snippet-collections", "serialization", "email-restrict-recipients"]
       :trial    false })
    (let [fut    (future
